...
void main() {
  ...
  test('시뮬레이터가 복사해 쓰는 상수가 실제 게임과 맞는다', () {
    ...
    // kUnitMultipliers — 유닛별 (피해, 공속) 배율. 등급별로 종류 순서 그대로다.
    // 이게 어긋나면 시뮬레이터가 초월을 과소·과대평가한다.
    expect(
      [
        for (final r in Rarity.values)
          [for (final u in kUnitsByRarity[r]!) [u.dmg, u.spd]],
      ],
      [
        [[1.0, 1.12], [0.85, 0.9], [0.7, 0.88], [0.8, 1.0]],
        [[1.05, 0.85], [0.9, 1.05], [0.8, 1.0], [1.38, 0.8]],
        [[1.28, 1.1], [0.95, 1.0], [1.1, 0.85], [0.95, 1.15]],
        [[1.2, 0.9], [1.05, 1.0], [1.05, 1.1], [1.15, 1.25]],
        [[1.25, 1.0], [1.15, 1.05], [1.95, 0.95], [1.05, 1.3]],
        [[1.3, 1.0], [1.45, 1.0], [1.2, 1.1]],
        [[1.6, 1.2], [1.75, 1.1], [2.1, 0.92]],
      ],
    );
