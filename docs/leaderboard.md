...
**남은 방어 하나** — API 키에 HTTP 리퍼러 제한(`ddai-min.github.io` 만 허용)을
걸면 남의 사이트에서 이 키로 쓰는 걸 줄일 수 있습니다. Google Cloud 콘솔의
«API 및 서비스 > 사용자 인증 정보» 에서 설정합니다. 브라우저 요청은 리퍼러를
위조할 수 있어 완전한 방어는 아니지만, 지나가는 스크립트는 걸러 줍니다.
