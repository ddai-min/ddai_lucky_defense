...
| 모드 | 곡선 | 30웨 | 60웨 | 100웨 | 중반 최저 여유 |
| --- | --- | --- | --- | --- | --- |
| 쉬움 | `90 × 1.10^(w-1)` | 1,428 | 24,912 | 1,127,505 | 2.39 |
| 보통 | 꺾인 곡선(아래) | 5,151 | 92,687 | 1,120,669 | **1.48** |
| 무한 | `90 × 1.17^(w-1)` | 8,543 | 948,874 | 506,573,914 | — |

쉬움과 보통은 **100 웨이브 체력이 거의 같습니다**(1,127,505 / 1,120,669). 끝나는
지점은 같되 가는 길이 다릅니다 — 보통은 30 웨이브에서 3.6배, 60 웨이브에서 3.7배
더 두껍습니다.
...
증가율을 웨이브마다 조금씩 꺾어(`1.18`에서 시작해 매 웨이브 `0.9875`배) 여유가
처음부터 끝까지 비슷하게 유지되도록 했습니다.

증가율을 조금만 더 올리면(`1.19`) 90 웨이브 언저리에서 막혀 도달률이 0%로
떨어집니다. 후반에는 플레이어가 한계에 닿아 더 셀 수 없기 때문에, 이 근처가
«깰 수 있는 가장 빡빡한 곡선» 입니다.
