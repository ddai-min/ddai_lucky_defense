...
| 모드 | **클리어 모드**(100 웨이브를 막아내면 승리)와 **무한 모드**(끝까지 얼마나) 중에서 인트로에서 고릅니다. |
...
static double enemyHp(int wave, {required bool endless}) =>                // 몬스터 체력 곡선
    90 * math.pow(endless ? 1.17 : 1.10, wave - 1);                       // 모드마다 다르다
...
**체력 곡선은 모드마다 다릅니다.** 클리어 모드는 100 웨이브에서 끝을 보라고 만든
모드이므로 거기까지 갈 수 있게 완만하고(×1.10), 무한 모드는 «얼마나 멀리 가나» 가
전부라 더 가파릅니다(×1.17). 같은 곡선을 쓰면 둘 중 하나가 망가집니다 — 무한 모드에
맞추면 100 웨이브를 아무도 못 깨고, 클리어 모드에 맞추면 무한 모드가 100 웨이브까지
밋밋해집니다.
